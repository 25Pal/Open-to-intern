...
const createIntern = async function (req, res) {
    try {
        ...
        if (!name || !email || !mobile || !collegeName) 
            return res.status(400).send({ status: false, message: "Please fill all the mandatory fields !!" });
        ...
        if (mobile) {
           ...
           if (mobile.length != 10) 
              return res.status(400).send({ status: false, message: "Please Check Your Number.Number should be in 10 digits.." })

            if (!validator.isNumeric(mobile)) 
              return res.status(400).send({ status: false, message: "Please Check Your Number" })
        ...
        if (checkEmailAndNum.length != 0) 
            return res.status(400).send({ status: false, message: "Email or mobile number is already exist" })
