    ...
    try {
        ...
        if (Object.keys(body).length != 0) {
            ...
            if (!validator.isAlphanumeric(req.body.name, "en-US", { ignore: '-' })) 
                return res.status(400).send({ status: false, message: "Please give valid  name" });
            
            if(validator.isNumeric(req.body.name)) 
                return res.status(400).send({ status: false, message: "Please give valid  name eg: 'xyz-35' " });

            if (!validator.isAlpha(req.body.fullName.split(' ').join(''))) 
                return res.status(400).send({ status: false, message: "Please give valid college name" });
            ...
            if ((link == url || link1 == url1) && (png == png1 || png == png2 || jpeg == jpeg1)) {
                ...
                if (dublicate.length != 0) 
                    return res.status(400).send({ status: false, message: "College Id or college is already in use !! Please use different name." })
...
const getCollegeDetails = async function (req, res) {
    ...
    try{ 
     ...
     if (typeof collegeName == 'undefined') 
        return res.status(400).send({ status: false, message: "Input something " })

     if (collegeName.trim() == "") 
        return res.status(400).send({ status: false, message: "College name can not be empty" })

     if (!validator.isAlphanumeric(collegeName, "en-US", { ignore: '-' })) 
     ...
     if (!checkCollege) 
        return res.status(400).send({ status: false, message: "No college exists with this college name or deleted !! " });
